...
export default function TransferTicket({ id }) {
  ...
  return (
    <>
      <div
        className="flex flex-row px-4 py-2 w-full group text-muted-foreground hover:text-muted-foreground hover:bg-accent hover:cursor-pointer"
        onClick={() => {
          setOpen(true);
        }}
      >
        <DocumentDuplicateIcon
          className="mr-3 h-5 w-5 text-muted-foreground group-hover:text-muted-foreground"
          aria-hidden="true"
        />
      ...
      <Transition.Root show={open} as={Fragment}>
        ...
        >
          <div className="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
            ...
            >
              <div className="inline-block align-bottom bg-card rounded-lg px-4 pt-5 pb-4 text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-xl h-64 sm:w-full sm:p-6">
                        ...
                        {({ open }) => (
                          <>
                            ...
                            <div className="mt-1 relative">
                              <Listbox.Button className="bg-card relative w-full border border-border rounded-md shadow-sm pl-3 pr-10 py-2 text-left cursor-default focus:outline-none focus:ring-1 focus:ring-ring focus:border-ring sm:text-sm">
                              ...
                              <Transition
                                ...
                                leaveTo="opacity-0"
                              >
                                <Listbox.Options className="absolute z-50 mt-1 w-full bg-card shadow-lg max-h-60 rounded-md py-1 text-base ring-1 ring-border overflow-auto focus:outline-none sm:text-sm">
                                  {users.map((user) => (
                                    <Listbox.Option
                                      key={user.id}
                                      className={({ active }) =>
