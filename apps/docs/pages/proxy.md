...
An example of this would be a helpdesk company running Mocha having it accessible to employees through their domain name.
...
```
https://mocha.example.com -> https://mochaapi.example.com -> nginx -> mocha docker
```
...
```
wget https://raw.githubusercontent.com/emberly/mocha/main/docker-compose.yml
```
...
This will pull the Mocha image & postgres and start the process of both containers. Once up, we have one finally more nginx config to take care of.
...
server {
    listen 80;
    listen [::]:80;
    server_name mochaapi.example.com;
    add_header Strict-Transport-Security "max-age=15552000; includeSubDomains" always;
...
You should now be able to see Mocha running on your chosen subdomain.
